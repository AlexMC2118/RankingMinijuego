<?php
  ...
  class Operaciones
  {
    ...
    public function eliminarRankingMenor() //Consulta encargada de eliminar todas las filas que tengan la puntuacion minima del minijuego
    {
      $consulta = "DELETE FROM partida WHERE  puntuacion = (SELECT MIN(puntuacion) FROM partida WHERE idMinijuego = 1);";
      $this->resultado= $this->hacerConsulta($consulta); //Enviamos la consulta al metodo query del objeto conexion (mysqli) y devolvera el array con la fila pedida
      return $this->resultado;
    }
